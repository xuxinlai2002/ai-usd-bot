...
class TelegramBot {
    bot;
    chatApiUrl;
    userTokens = new Map(); // 存储用户的认证 token
    constructor() {
        // 验证必要的环境变量
    ...
    async processChatMessage(ctx, userMessage, userToken) {
        try {
            // 发送"正在处理"消息
            const processingMsg = await ctx.reply('⏳ 正在处理您的请求...');
            // 调用聊天接口
    ...
    async callChatApi(userMessage, userToken) {
        try {
            const messages = [
                {
                    role: 'user',
            ...
            const response = await axios.post(this.chatApiUrl, {
                messages,
                auth: userToken
            }, {
                headers: {
